// BeadWatch — Instrument Comparison Logic
...
(function () {
    ...
    var snCanvas = document.getElementById('sn-chart');
    var snPlaceholder = document.getElementById('sn-placeholder');
    var snSubtitle = document.getElementById('sn-subtitle');
    var ncCanvas = document.getElementById('nc-chart');
    var ncPlaceholder = document.getElementById('nc-placeholder');
    var ncSubtitle = document.getElementById('nc-subtitle');
    var countsCanvas = document.getElementById('counts-chart');
    var countsPlaceholder = document.getElementById('counts-placeholder');
    var countsSubtitle = document.getElementById('counts-subtitle');
    ...
    function addRefLines(chart, color) {
        // Collect all non-null values across all instrument datasets
        var all = [];
        ...
        // Axis bounds at mean ± 5 SD (wider than dashboard's 3 SD to suit
        // multi-instrument spread); red fill at ± 2 SD, clamp at ± 5 SD.
        ...
        // Colour outliers solid red and clamp beyond ± 5 SD
        chart.data.datasets.forEach(function (ds) {
    ...
    function buildDatasets(chart, dataByInstrument, valueKey, minKey, maxKey) {
        // Collect all unique dates in order across all instruments
        var allDates = [];
        var dateSet = {};
        Object.keys(dataByInstrument).forEach(function (inst) {
            dataByInstrument[inst].forEach(function (d) {
        ...
        var datasets = [];
        var instruments = Object.keys(dataByInstrument).sort();
        instruments.forEach(function (inst, i) {
            var colour = nameToColour(inst, instruments.length, i);
            var dataMap = {};
            dataByInstrument[inst].forEach(function (d) {
    ...
    // ---- Group API response by instrument ----
    function groupByInstrument(data) {
        var grouped = {};
        data.forEach(function (d) {
            if (!grouped[d.instrument]) grouped[d.instrument] = [];
            grouped[d.instrument].push(d);
        });
        return grouped;
    }
    ...
    async function loadAllCharts() {
        ...
        try {
            // Fetch instruments first to check cache readiness
            var instruments = await apiGet('/api/dashboard/qc/instruments?days=' + currentDays);

            if (!instruments.length) {
                // Cache is empty (populating in background) — show spinner and auto-retry
                showLoading(snCanvas, snPlaceholder, 'Populating instrument cache \u2014 this may take a few minutes\u2026');
                showLoading(ncCanvas, ncPlaceholder, 'Populating instrument cache \u2014 this may take a few minutes\u2026');
                showLoading(countsCanvas, countsPlaceholder, 'Populating instrument cache \u2014 this may take a few minutes\u2026');
                // Do NOT set loadedKey — allow retry
            ...
            var results = await Promise.all([
                apiGet('/api/dashboard/qc/instrument-sn-ratio?days=' + currentDays),
                apiGet('/api/dashboard/qc/instrument-nc?days=' + currentDays),
                apiGet('/api/dashboard/qc/instrument-bead-counts?days=' + currentDays),
            ]);
            ...
            if (snChart && snData.length) {
                buildDatasets(snChart, groupByInstrument(snData), 'sn_median', 'sn_min', 'sn_max');
            ...
            if (ncChart && ncData.length) {
                buildDatasets(ncChart, groupByInstrument(ncData), 'nc_median', 'nc_min', 'nc_max');
            ...
            if (countsChart && countsData.length) {
                buildDatasets(countsChart, groupByInstrument(countsData), 'median_count', 'min_count', 'max_count');
            ...
            showPlaceholder(snCanvas, snPlaceholder, 'Failed to load instrument data');
            showPlaceholder(ncCanvas, ncPlaceholder, 'Failed to load instrument data');
            showPlaceholder(countsCanvas, countsPlaceholder, 'Failed to load instrument data');
        }
    }
    ...
    var refreshBtn = document.getElementById('inst-refresh-btn');
    if (refreshBtn) {
